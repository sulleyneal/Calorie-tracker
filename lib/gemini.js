  ...
  "items": [
    {
      ...
      "userStated": false,                // true if the user explicitly stated the calories/macros ("about 200 calories") — then kcal MUST be their number
      "usdaQuery": "croissant almond"    // USDA FoodData Central search query; "" for branded or userStated items
    }
  ],
  "reply": "one warm, concise sentence reacting to the meal — like a supportive friend, never preachy. NEVER make arithmetic claims (percentages, 'halfway there', calories remaining) — the app displays exact numbers itself; if you mention a number, only repeat one given in Context verbatim."
}
...
MACRO SHAPE — macros must fit the FOOD, not just sum to the calories:
pizza ≈ P12 C36 F13 per slice (never P3 on a full slice); chicken wings are
protein+fat (one wing with sauce ≈ 90-110 cal, 6 wings ≈ 550-650 cal);
plain fruit and vegetables have near-zero fat. Double-check protein last.`;

const PHOTO_SYSTEM = PARSE_SYSTEM + `

PHOTO MODE — the user sent a PHOTO of their meal instead of (or along with) text:
- If there is NO identifiable food in the photo, the reply must SAY SO plainly
  ("I couldn't spot any food in that one...") and invite them to describe the
  meal in words. Never imply anything was logged when items is empty.
